...
if __name__ == '__main__':
    ...
    axes[1].scatter(s.X[:, 1], s.X[:, 2], c=s.y, s=7, marker='s')
    axes[1].scatter(X_test[:, 0], X_test[:, 1], c=y_test, s=7, marker='^')
    plt.legend(["training set", "test set"])
    epochs = np.arange(s.epochs)
    ...
    scatter2 = axes[2].scatter([], [], s=3, c='b')
    scatter3 = axes[2].scatter([], [], s=3, c='r')
    plt.legend(["training set", "test set"])
    ...
    def animate(i):
        ...
        for c in cont.collections:
            c.remove()
        line0.set_data(epochs[:i], s.loss[:i])  # update data

        scatter2.set_offsets(np.stack((epochs[:i], ac_train[:i]), axis=1))  # update data
        scatter3.set_offsets(np.stack((epochs[:i], ac_test[:i]), axis=1))
    ...
    ani = animation.FuncAnimation(fig, animate, interval=1, frames=s.epochs, repeat=False)
